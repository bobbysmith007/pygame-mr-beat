...
class Main:
	...
	def TapTempo(self, widget):
		...
		tempo_last_tap = ((1000/TIMER_ACCURACY)*60) / self.taptempo_ticks
		average = (self.wTree.get_widget("tempo").get_value() + tempo_last_tap) / 2
		self.wTree.get_widget("tempo").set_value(average)
		self.ticks = 0
		self.taptempo_ticks = 0
		self.beat = 0
	def ticking_loop(self):
		...
		while self.playing:
			if(time.time() >= self.last_time + 0.025):
				self.last_time += 0.025
				self.tick_stuff()
	...
	def tick_stuff(self):
		...
		#Check if the user is tapping the tempo
		if self.taptempo_mode == True:
			self.taptempo_ticks += 1
			#Shut off taptempo mode after 5 seconds (NOBODY would tap 12 time!)
			if self.taptempo_ticks > (1000/TIMER_ACCURACY) * 5:
				self.taptempo_mode = False
				self.taptempo_ticks = 0
